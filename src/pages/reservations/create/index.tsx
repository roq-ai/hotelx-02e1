...
function ReservationCreatePage() {
  ...
  const formik = useFormik<ReservationInterface>({
    initialValues: {
      check_in_date: new Date(new Date().toDateString()),
      check_out_date: new Date(new Date().toDateString()),
        ...
        <form onSubmit={formik.handleSubmit}>
          <FormControl id="check_in_date" mb="4">
            ...
            <Box display="flex" maxWidth="100px" alignItems="center">
              <DatePicker
                ...
                selected={formik.values?.check_in_date ? new Date(formik.values?.check_in_date) : null}
                onChange={(value: Date) => formik.setFieldValue('check_in_date', value)}
              />
              <Box zIndex={2}>
                <FiEdit3 />
              </Box>
          ...
          <FormControl id="check_out_date" mb="4">
            ...
            <Box display="flex" maxWidth="100px" alignItems="center">
              <DatePicker
                dateFormat={'dd/MM/yyyy'}
                selected={formik.values?.check_out_date ? new Date(formik.values?.check_out_date) : null}
                onChange={(value: Date) => formik.setFieldValue('check_out_date', value)}
              />
              <Box zIndex={2}>
                <FiEdit3 />
              </Box>
